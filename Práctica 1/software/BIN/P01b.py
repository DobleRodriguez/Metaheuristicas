...
def local_search(data, const, ncluster, scale_factor, randgen):
    ...
    for i in np.nditer(np.arange(len(solution))):
        ...
        #print(f"solution[i] = {solution[i]} solution[ml] = {solution[ml_conflicting_data]} solution[cl] = {solution[cl_conflicting_data]}")
        #input(print(f"Infeas = {infeasibility}"))
    ...
    while(not best_solution):
        ...
        for index, cluster in neighbors:
            ...
            #print("ML")
            #print(ml_conflicting_data)
            #print(solution[ml_conflicting_data])
            #print(np.flatnonzero(solution[index] != solution[ml_conflicting_data]))
            #print(np.count_nonzero(solution[index] != solution[ml_conflicting_data]))
            #print("CL")
            #print(cl_conflicting_data)
            #print(solution[cl_conflicting_data])
            #print(np.flatnonzero(solution[index] == solution[cl_conflicting_data]))
            #print(np.count_nonzero(solution[index] == solution[cl_conflicting_data]))
            #print(f"Orig {original_infeas}")
            #print("CAMBIADO")
            #print("ML")
            #print(new_possibility[ml_conflicting_data])
            #print(np.flatnonzero(cluster != new_possibility[ml_conflicting_data]))
            #print(np.count_nonzero(cluster != new_possibility[ml_conflicting_data]))
            #print("CL")
            #print(new_possibility[cl_conflicting_data])
            #print(np.flatnonzero(cluster == new_possibility[cl_conflicting_data]))
            #print(np.count_nonzero(cluster == new_possibility[cl_conflicting_data]))
            #print(f"Nueva {infeas_change}")
            #input(print(new_infeas))
...
with open(pl.Path(__file__).parent / f"solutions_P01b.txt", 'w+') as sol_file:
    sol_file.write(
...
for percent,dataset,seed,ncluster in values:
    ...
    general_desviation, infeasibility, objective = weak_const_kmeans(data, const, int(ncluster), scale_factor, randgen)
    toc = time.perf_counter()
    func_name = "Greedy kmedias"
    with open(pl.Path(__file__).parent / f"solutions_P01b.txt",'a+') as sol_file:
        sol_file.write(
    ...
    tic = time.perf_counter()
    general_desviation, infeasibility, objective = local_search(data, const, int(ncluster), scale_factor, randgen)
    toc = time.perf_counter()
